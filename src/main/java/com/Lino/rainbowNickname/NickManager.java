...
public class NickManager {
    ...
    public void enableNick(Player player) {
        ...
        // Skip TextDisplay if using PlaceholderAPI
    ...
    public void disableNick(Player player) {
        // Only disable TextDisplay if not using PlaceholderAPI
        if (!plugin.isUsingPlaceholderAPI()) {
    ...
    public boolean hasNick(Player player) { 
        // If using PAPI, check animation type instead of TextDisplay
        if (plugin.isUsingPlaceholderAPI()) {
    ...
    /**
     * Gets the formatted nickname for a player with the current phase.
     * Returns the player's original name if no animation is set.
     */
    public String getFormattedName(Player player, float phase) {
        if (!playerAnimations.containsKey(player.getUniqueId())) {
